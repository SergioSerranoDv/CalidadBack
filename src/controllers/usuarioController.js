...
const registrar = async (req, res) => {
  ...
  if (
    ...
    return res.status(400).json({
      msg: "La contraseña debe tener al menos 8 caracteres, una letra mayúscula, un número y un símbolo",
    })
  }
...
const updateParameters = async (req, res) => {
  try {
    ...
    usuario.parameters = parameters
    const updatedUserWithParameters = await usuario.save()
    return res.json(updatedUserWithParameters)
  } catch (error) {
    console.log(error)
    res.status(404).json({ msg: "Error al obtener el perfil del usuario" })
  }
